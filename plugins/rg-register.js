...
let handler = async function (m, { conn, text, usedPrefix, command }) {
 ...
 if (user.registered === true) throw `🗂 ${mssg.regIsOn}\n\n${usedPrefix}unreg <sn>`
  ...
  let te = `🗂 ${mssg.useCmd}: *${usedPrefix + command} ${mssg.name}+${mssg.age}+${mssg.gender}*\n📌 ${mssg.example}: *${usedPrefix + command}* Fulano+18+M\n\n▢ ${mssg.genderList}:\n*- M* = ${mssg.man}\n*- F* ${mssg.woman}\n*- N* = ${mssg.other}`
  ...
  if (age < 10) throw '🚼 Você era pra estar assistindo mundo bita'
...
handler.help = ['reg'].map(v => v + ' <nome.idade.genero>')
handler.tags = ['rg']
